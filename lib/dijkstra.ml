...
let%test_module "make_initial_eki_list" = (module struct
  let%test _ = make_initial_eki_list [
...
let%test_module "seiretsu" = (module struct
  let%test _ = seiretsu [
    {kanji="池袋"; kana="いけぶくろ"; romaji="ikebukuro"; shozoku="丸ノ内線"};
    {kanji="新大塚"; kana="しんおおつか"; romaji="shinotsuka"; shozoku="丸ノ内線"};
    {kanji="茗荷谷"; kana="みょうがだに"; romaji="myogadani"; shozoku="丸ノ内線"};
...
let%test_module "koushin" = (module struct
...
let%test_module "saitan_wo_bunri" = (module struct
...
let%test_module "dijkstra" = (module struct
  let%test _ = dijkstra "茗荷谷" "本郷三丁目" = {namae="本郷三丁目"; saitan_kyori=2.6; temae_list=["本郷三丁目"; "後楽園"; "茗荷谷"]}
  (* saitan_kyori doesn't matter. *)
  let%test _ = (dijkstra "茗荷谷" "新宿御苑前").namae = "新宿御苑前"
  let%test _ = (dijkstra "茗荷谷" "新宿御苑前").temae_list = ["新宿御苑前"; "四谷三丁目"; "四ツ谷"; "市ヶ谷"; "飯田橋"; "後楽園"; "茗荷谷"]
end)
